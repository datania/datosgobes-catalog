...
BASE_URL = "https://datos.gob.es/apidata/catalog/publisher"
PAGE_SIZE = 50
CONCURRENT_PAGES = 20
MAX_RETRIES = 3
...
async def fetch_page(client: httpx.AsyncClient, page: int) -> Optional[dict]:
    """Fetch a single page of publisher listings with retry and exponential backoff"""
...
def get_publisher_path(publisher_id: str) -> Path:
    """Generate path for a publisher JSON file"""
    if not publisher_id:
        return Path("catalog/publisher/misc/unknown.json")
    
    filename = f"{publisher_id}.json"
    return Path("catalog/publisher") / filename


async def export_all_publishers():
    """Export all publishers from datos.gob.es to local JSON files"""
    print("Starting publisher export...")
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as client:
        ...
        while empty_pages < 3:
            ...
            # Process all publishers from fetched pages
            found_any = False
            publishers_to_save = []
            ...
            # Save publishers
            for publisher_id, data in publishers_to_save:
                try:
                    file_path = get_publisher_path(publisher_id)
                    ...
                    print(f"Error saving publisher {publisher_id}: {e}")

            # Progress update
            if publishers_to_save:
                elapsed = time.time() - start_time
                rate = total_saved / elapsed if elapsed > 0 else 0
                print(
                    f"Pages {page}-{page + CONCURRENT_PAGES - 1}: "
                    f"Saved {total_saved}/{total_found} publishers ({rate:.1f}/sec)"
                )
    ...
    elapsed = time.time() - start_time
    print("\nExport complete!")
    print(f"Total publishers: {total_found}")
    print(f"Successfully saved: {total_saved}")
    print(f"Failed: {total_found - total_saved}")
    print(f"Time: {elapsed:.1f} seconds ({total_saved / elapsed:.1f} publishers/sec)")
    print(f"Location: {Path('catalog/publisher').absolute()}/")


if __name__ == "__main__":
    asyncio.run(export_all_publishers())